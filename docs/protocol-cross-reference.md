...
Note: in this document, `NavigationNextTurnEvent` is Sony-symbol terminology. The canonical repo now distinguishes deprecated `NavigationTurnEvent` (`0x8004`, 16.1-only evidence), `LegacyNavigationTurnEvent` (`0x8005` in 16.2), and `NavigationNextTurnDistanceEvent` (`0x8007`).
...
| NavigationNextTurnEvent | Sony-symbol family for flat next-turn guidance; canonical repo splits this into deprecated `NavigationTurnEvent` (`0x8004`, 16.1-only evidence) and `LegacyNavigationTurnEvent` (`0x8005` in 16.2) |
...
|---------|----------------------|-------|
| Navigation status (10) | NavigationNextTurnEvent, NavigationNextTurnDistanceEvent | Turn-by-turn in native UI (Sony naming; canonical repo further splits flat turn events into deprecated `NavigationTurnEvent` and `LegacyNavigationTurnEvent`) |
