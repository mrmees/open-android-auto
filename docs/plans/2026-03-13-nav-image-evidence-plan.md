...
| Q4 | 16.2 has a native image-bearing successor path | Needs better evidence | 16.2 still keeps app-side `turnImage` bytes on `NavigationStep.f20729c` and parcels them as field `5` (`NavigationStep.java:25`, `NavigationStep.java:45`, `NavigationStep.java:82-88`), but the semantic `32774` builder never reads that field and only copies maneuver/text/lanes/road-info data into `vzg` (`hlj.java:361-545`). The legacy branch under `m18759z(carInfo)` still forwards `navigationStep2.f20729c` or fallback `bArr` into `mo18767n(...)` (`hlj.java:643-645`, `hlj.java:805-813`), yet `mo18767n(...)` is not decompiled in this dump (`hlj.java:929-934`). Task 7 exhausted the named image-negotiation search in `p000/*.java`: there were no `NEXT_TURN_IMAGE`, `NavigationImageOptions`, or `colour_depth` hits, and the only `turnImage` / `nextTurnImage` hits were projected-UI model/style strings in `ggf.java`, `ggj.java`, and `ggo.java`. That rules out a named 16.2 successor path, but not an opaque legacy send hidden inside the undecompiled `mo18767n(...)` body. |
| Q5 | `NEXT_TURN_IMAGE` is reachable in 16.2 | Rejected | Task 7 source search over 16.2 `p000/*.java` found no `NEXT_TURN_IMAGE`, `NavigationImageOptions`, or `colour_depth` references at all; the only matching terms were projected-UI `turnImage` / `nextTurnImage` names in `ggf.java:4-60`, `ggj.java:4-64`, and `ggo.java:71-77`. The native nav sender still exposes only `32773` from `ian.m20073h(...)` (`ian.java:118-148`) plus `32774`, `32775`, and `32776` from `hlj` (`hlj.java:304-322`, `hlj.java:635`, `hlj.java:939-954`); the extra `32772` / `32774` hits from the broad search are in `CAR.INPUT` / `CAR.GAL.CAR_CONTROL`, not navigation (`hlg.java:24`, `hlg.java:494-507`; `hyc.java:1-18`; `hlb.java:27`, `hlb.java:560-584`). Repo proto placeholders still define `NEXT_TURN_IMAGE = 2` and `NavigationImageOptions`, but Task 7 found no 16.2 sender path that uses them. |
| Q6 | `junctionImage` or `lanesImage` reach native nav transport in 16.2 | Rejected | In 16.2 projected navigation models, `Maneuver` stores a `CarIcon mIcon` (`Maneuver.java:60-70`, `Maneuver.java:100-101`), `Step` stores `CarIcon mLanesImage` (`Step.java:15-27`, `Step.java:49-54`), and `RoutingInfo` stores `CarIcon mJunctionImage` (`RoutingInfo.java:13-17`, `RoutingInfo.java:47-56`). Projected UI renderer `jbl` consumes those assets directly via `routingInfo.getJunctionImage()`, `currentStep.getManeuver().getIcon()`, `currentStep.getLanesImage()`, and `nextStep.getManeuver().getIcon()` (`jbl.java:548-606`). No native 16.2 sender path consumes these `CarIcon` fields: semantic `32774` stays image-free (`hlj.java:361-635`; `vzg.java:13-25`, `vzg.java:44-45`), and the only visible legacy helper signature accepts a single `byte[]` turn image rather than lane/junction `CarIcon` assets (`hlj.java:643-645`, `hlj.java:805-813`, `hlj.java:929-934`). |
...
- Last completed task: `Task 9 - cross-version nav image evidence matrix`
- Last verified claim: `The 16.1 vs 16.2 matrix is now normalized: 16.1 still has confirmed legacy 32772 turn-image delivery with local fallback synthesis, while 16.2 has confirmed semantic-only 32774 delivery plus projected-only CarIcon assets, with only the opaque legacy mo18767n(...) path still unclosed`
- Evidence files:
...
- Next unanswered question: `Which canonical nav docs/comments need updates now that Q5 and Q6 are rejected, Q4 and Q7 are bounded, and the matrix is in place?`
- Next command to run: `rg -n "NEXT_TURN_IMAGE|turn_icon|junctionImage|lanesImage|Visual Payloads" /home/matt/claude/personal/openautopro/open-android-auto/.worktrees/nav-image-evidence-20260313/docs/channels/nav.md /home/matt/claude/personal/openautopro/open-android-auto/.worktrees/nav-image-evidence-20260313/oaa/navigation`
...
| Legacy image-bearing sender | Confirmed: `32772`; `hkx.n(...)` writes bytes into `vzm.f` | Needs better evidence: `m18759z(...)` still passes a `byte[]` into undecompiled `mo18767n(...)` | 16.2 retained legacy byte plumbing but the sender body is still opaque. |
| Local fallback image generation | Confirmed: `hkx.n(...)` synthesizes `da_turn_*` assets locally | Needs better evidence: helper body unavailable | Only the 16.1 fallback mechanism is source-backed today. |
...
| Native-wire lane/junction/turn image payloads | Turn-image bytes confirmed on `32772`; no lane/junction payload evidence | No lane/junction payload evidence; turn-image wire still unclosed because `mo18767n(...)` is opaque | Distinguishes native-wire turn images from projected UI assets. |
