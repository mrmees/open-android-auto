...
| Q7 | Cross-version capability gates controlling nav image delivery are understood | Confirmed | 16.1 sender gating is source-backed: `CarInfo.e` / `f` are `headUnitProtocolMajorVersionNumber` / `headUnitProtocolMinorVersionNumber` (`ijk.java:59`); `hkx.x(carInfo)` treats HU protocol `>= 1.6` as modern, `y(carInfo) = this.e || x(carInfo)`, and `z(carInfo) = !x(carInfo)` (`hkx.java:47-60`, `hkx.java:304-308`, `hkx.java:586-592`). `hkx.a(wbwVar)` shows `(wbwVar.b & 128)` is the instrument-cluster descriptor carrying `vzr` image options (`hkx.java:70-125`). Post-plan simple JADX over 16.1 `classes.dex` recovered the missing `iks.g(...)` branch and showed `z5` stays true only when the incoming SDR lacks such a `wbw.j` / bit-`128` entry, so clustersim injects one, writes `poe.b = z5`, and constructs `ikr(..., z5)`; otherwise it forces `z5 = false` (exact command/result recorded in the 2026-03-13 Q7-closure handoff entry). 16.2 mirrors the same shape: `hlj.mo18712a(wbmVar)` shows `(wbmVar.f74994b & 128)` is the instrument-cluster descriptor carrying `vzd` image options (`hlj.java:116-152`); `m18757x(carInfo)`, `m18758y(carInfo)`, and `m18759z(carInfo)` keep the same `>= 1.6` threshold structure (`hlj.java:95-110`, `hlj.java:361-365`, `hlj.java:643-645`). Post-plan simple JADX over 16.2 `classes.dex` recovered `ilf.g(...)` and showed `z5` stays true only when the incoming SDR lacks that `wbm.f75002j` / bit-`128` entry, so clustersim injects one, writes `pnl.f57495b = z5`, and constructs `ile(..., z5)`; otherwise it forces `z5 = false` (exact command/result recorded in the 2026-03-13 Q7-closure handoff entry). Therefore `poe.b` / `pnl.f57495b` / `this.e` / `f34211e` is a clustersim-specific rich-nav capability override tied to synthetic instrument-cluster descriptor injection, not a second HU-version field. |
...
- Last completed task: `Q7 capability-gate semantics closure`
- Last verified claim: `The 16.1/16.2 clustersim vendor bit becomes true only when the SDR munger injects a missing instrument-cluster descriptor (the wbw/wbm bit-128 entry carrying vzr/vzd image options), and hkx/hlj use that bit as a rich-nav capability override`
- Evidence files:
  - `/home/matt/claude/personal/openautopro/open-android-auto/analysis/android_auto_16.2.660604-release_162660604/apk-source/sources/p000/iom.java`
  - `/home/matt/claude/personal/openautopro/open-android-auto/analysis/android_auto_16.2.660604-release_162660604/apk-source/sources/p000/hmi.java`
  - `/home/matt/claude/personal/openautopro/open-android-auto/analysis/android_auto_16.2.660604-release_162660604/apk-source/sources/p000/pnl.java`
  ...
  - `/home/matt/claude/personal/openautopro/open-android-auto/analysis/android_auto_16.2.660604-release_162660604/apk-source/sources/p000/ilf.java`
  ...
  - `/home/matt/claude/personal/openautopro/openauto-prodigy/analysis-projection/android_auto_16.1.660414-release_161660414/apk-source/sources/defpackage/poe.java`
  - `/home/matt/claude/personal/openautopro/openauto-prodigy/analysis-projection/android_auto_16.1.660414-release_161660414/apk-source/sources/defpackage/iks.java`
  - `/home/matt/claude/personal/openautopro/openauto-prodigy/analysis-projection/android_auto_16.1.660414-release_161660414/apk-source/resources/classes.dex`
  - `docs/session-handoffs.md`
- Next unanswered question: `None in the current nav-image evidence ledger; optional follow-up is only a historical-doc sweep for stale “Q7 open” wording.`
- Next command to run: `rg -n "Q7 .*Confirmed|synthetic instrument-cluster descriptor|rich-nav capability override" /home/matt/claude/personal/openautopro/open-android-auto/.worktrees/nav-image-evidence-20260313/docs/plans/2026-03-13-nav-image-evidence-plan.md /home/matt/claude/personal/openautopro/open-android-auto/.worktrees/nav-image-evidence-20260313/docs/plans/2026-03-13-nav-image-evidence-design.md /home/matt/claude/personal/openautopro/open-android-auto/.worktrees/nav-image-evidence-20260313/docs/session-handoffs.md`
...
| Capability gate override | Confirmed: `x(carInfo)` still uses HU protocol `>= 1.6`, while clustersim override `poe.b` becomes true only when the SDR munger injects a missing `wbw.j` / bit-`128` instrument-cluster descriptor | Confirmed: `m18757x(carInfo)` still uses HU protocol `>= 1.6`, while clustersim override `pnl.f57495b` becomes true only when the SDR munger injects a missing `wbm.f75002j` / bit-`128` instrument-cluster descriptor | In both versions the override feeds `hkx.y(...)` / `hlj.m18758y(...)`, so the semantic rich-nav gate is clustersim-specific rather than a second HU-version field. |
